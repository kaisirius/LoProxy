#include <blocking-server/BlockingServer.hpp>
...
std::unique_ptr<BlockingServer> BlockingServer::BlockingServerInstance = nullptr;
std::mutex BlockingServer::mtx;
...
BlockingServer* BlockingServer::getInstance() {
    if(BlockingServerInstance == nullptr) {
        std::lock_guard<std::mutex> lock(mtx);
        if(BlockingServerInstance == nullptr) {
            BlockingServerInstance = std::unique_ptr<BlockingServer>(new BlockingServer());
        }
    }
    return BlockingServerInstance.get();
}

void BlockingServer::init() {
    std::cout << "Starting BlockingServer at 127.0.0.1" << "\n";
    ...
    std::cout << "BlockingServer started listening on port 8080, Ready to accept connections." << "\n";
...
void BlockingServer::sendAllBytes(int connectedSocket, ssize_t bytesToSend, char buff[]) {
...
BlockingServer::~BlockingServer() {
    std::cout << "Closing BlockingServer..." << "\n";
